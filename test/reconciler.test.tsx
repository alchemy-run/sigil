...
test("update child", () => {
  ...
  expect(stdoutActual.write.mock.lastCall![0]).toBe(stdoutExpected.write.mock.lastCall![0]);
...
test("support suspense", async () => {
  ...
  expect(stdout.write.mock.lastCall![0]).toBe("Loading");
...
test("support suspense with concurrent mode", async () => {
  ...
  expect(stdout.write.mock.lastCall![0]).toBe("Hello Concurrent World");
});
